...
#include <core/scoring/shape/zernike/ZernikeMoments.inl.hh>
#include <iostream>
#include <fstream>
 ...
 */
template<class VoxelT, class MomentT>
void ZernikeMoments<VoxelT,MomentT>::SaveGrid (ComplexT3D& _grid, std::string filename) {

    int dimX = _grid.size ();
    int dimY = _grid[0].size ();
    int dimZ = _grid[0][0].size ();

    std::ofstream gridvals;
    gridvals.open(filename);
    gridvals << "x,y,z,real,imag" << std::endl;
    int x, y, z;
    for (x = 0; x < dimX; x++) {
        for (y = 0; y < dimY; y++) {
            for (z = 0; z < dimZ; z++) {
                auto real = _grid[x][y][z].real();
                auto imag = _grid[x][y][z].imag();
                gridvals << x << "," << y << "," << z << "," << real << "," << imag << "\n";
            }
        }
    }
    // write surface data
...
template class ZernikeMoments<double,double>;
template struct  ComplexCoeff<double>;
}
}
}
}
